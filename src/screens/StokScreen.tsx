...
export type StokSubTab = 'KATALOG' | 'FORECAST' | 'OPNAME' | 'TRANSFER';

interface StokScreenProps {
  onNavigate?: (tab: TabDestination) => void;
}

export const StokScreen: React.FC<StokScreenProps> = ({ onNavigate }) => {
  const { 
    products, 
    inventoryLayers, 
    stockOpnames,
    ...
    <div className="p-4 sm:p-5 space-y-4 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-200/80 pb-3">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl flex-wrap">
          ...
          <div 
            className="px-2.5 py-1.5 rounded-lg text-xs font-medium text-slate-400 bg-slate-100 flex items-center gap-1.5 cursor-not-allowed opacity-60"
            title="Fitur AI Forecast dinonaktifkan untuk menjaga kepatuhan Offline-First APK"
          >
            <Sparkles className="w-3.5 h-3.5 text-slate-400" />
            <span>AI Forecast (Segera Hadir)</span>
          </div>
                      ...
                      <div className="flex items-center justify-between text-slate-600 font-semibold text-[11px]">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] transition cursor-pointer shadow-2xs"
                          >
                            <Sparkles className="w-3 h-3 text-amber-300" />
                            <span>Prediksi AI Stok</span>
                          </button>
      ...
      {/* SUB-TAB 2: AI INVENTORY FORECAST & REORDER RECOMMENDATIONS (OFFLINE COMPLIANT) */}
      {activeTab === 'FORECAST' && (
        <div className="p-8 text-center bg-white rounded-2xl border border-slate-200 shadow-2xs space-y-3 m-3">
          <div className="w-12 h-12 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center mx-auto">
            <Sparkles className="w-6 h-6" />
          </div>
          <h3 className="text-sm font-bold text-slate-800">AI Inventory Forecast (Segera Hadir)</h3>
          <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
            Fitur analisis AI server dinonaktifkan untuk menjaga kepatuhan arsitektur Offline-First murni aplikasi Android ERP (bebas dependensi runtime server Express/Node.js).
          </p>
      ...
      {/* SUB-TAB 3: STOCK OPNAME */}
      {activeTab === 'OPNAME' && (
        <StockOpnameTab />
      )}

      {/* SUB-TAB 4: TRANSFER ANTAR LOKASI */}
      {activeTab === 'TRANSFER' && (
        <StockTransferTab />
      )}
